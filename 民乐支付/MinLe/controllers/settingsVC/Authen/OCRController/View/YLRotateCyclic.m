...
@property (nonatomic, strong) UIColor *backgroundCyclicColor;
...
-(instancetype)initWithFrame:(CGRect)frame colors:(NSArray *)colors drawDuration:(CGFloat)drawDuration sapceAngle:(CGFloat)spaceAngle
{
    if (self = [super initWithFrame:frame]) {
        self.colors = colors;
        self.backgroundCyclicColor = [ColorsUtil colorWithHexString:@"#d2d8dc"];
...
- (void)drawRotateCyclic
{
    ...
    if (_bottomPath == nil) {
        UIBezierPath* bottomPath = [UIBezierPath bezierPathWithArcCenter:center radius:radious startAngle:-M_PI_2 endAngle:M_PI * 2 clockwise:YES];
