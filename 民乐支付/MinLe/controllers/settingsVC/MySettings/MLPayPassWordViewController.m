...
@interface MLPayPassWordViewController ()
{
    NSTimer *time;
    int myTime;
}
...
@implementation MLPayPassWordViewController
...
- (MLMinLeTextField *)newsPassTextField
{
    if (!_newsPassTextField) {
        _newsPassTextField = [[MLMinLeTextField alloc] initWithFrame:CGRectMake(15, CGRectGetMaxY(self.yzmBut.frame)+20, CGRectGetWidth(self.layerView.frame)-30, TextFieldHeight)];
        _newsPassTextField.placeholder = [CommenUtil LocalizedString:@"Setting.SetupPayPassword"];
...
- (MLMinLeTextField *)turePassTextField
{
    if (!_turePassTextField) {
        _turePassTextField = [[MLMinLeTextField alloc] initWithFrame:CGRectMake(15, CGRectGetMaxY(self.newsPassTextField.frame)+20, CGRectGetWidth(self.layerView.frame)-30, TextFieldHeight)];
        _turePassTextField.placeholder = [CommenUtil LocalizedString:@"Setting.TurePayPassword"];
...
{
    if (!_naView) {
        _naView = [[MLMyNavigationView alloc] initWithFrame:CGRectMake(0, 0, widthss, 264) withColor:[UIColor whiteColor] withTitle:[CommenUtil LocalizedString:@"Setting.SetupPayPassword"]];
    }
    return _naView;
}
...
- (void)playAction:(UIButton *)sender
{
    if (self.yzmTextField.text.length == 0 ||
        ...
        NSDictionary *dic = @{@"token":[[NSUserDefaults standardUserDefaults] objectForKey:@"token"],
                              ...
                              @"pay_pass":self.newsPassTextField.text,
                              @"com_pay_pass":self.turePassTextField.text,
                              @"prefix":_countryCode};
        [[MCNetWorking sharedInstance] createPostWithUrlString:pay_passURL withParameter:dic withComplection:^(id responseObject) {
            [[MBProgressHUD shareInstance] ShowMessage:responseObject[@"msg"] showView:nil];
            if ([responseObject[@"status"] isEqual:@1]) {
                [self.navigationController pushViewController:[[MLSettingsViewController alloc] init] animated:YES];
            } else {
