...
@property (nonatomic, strong) NSString *proString;//接受设置分隔符的
    ...
    else
    {
        ...
        if([self.format rangeOfString:@"%(.*)d" options:NSRegularExpressionSearch].location != NSNotFound || [self.format rangeOfString:@"%(.*)i"].location != NSNotFound )
        {
            ...
            if (self.positiveFormat.length > 0) {
                ...
                [fomatter setPositiveFormat:self.positiveFormat];
                self.text = [NSString stringWithFormat:@"￥%@",[fomatter stringFromNumber:[NSNumber numberWithFloat:[str floatValue]]]];
            } else {
                NSString *str = [NSString stringWithFormat:self.format,value];
                self.text = [NSString stringWithFormat:@"￥%@",str];
            }
